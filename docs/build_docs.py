...
# The concept diagrams live at the repository root in `images/`, beside the
# prompt file that produced them, because they are not documentation-only: the
# Notion chapter uses the same files. A second copy kept under docs/ by hand
# would be the one that goes stale.
#
# WHY THEY ARE COPIED RATHER THAN POINTED AT. Quarto resolves `resources`
# inside the project directory and will not follow a `../` path out of it, and
# the PDF is rendered from a file concatenated into docs/, so a path that
# escapes upward breaks in both outputs at once. Copying costs 3 MB and makes
# `images/name.png` mean the same thing from every page and from the PDF.
IMAGES_SRC = ROOT / "images"
IMAGES_DST = HERE / "images"
...
def sync_images() -> int:
    """Copy the concept diagrams into the Quarto project, and return the count.

    Deletes a copy whose source is gone. A diagram removed upstream that stayed
    behind here would keep being served by the site and bound into the PDF with
    no prompt behind it, which is the failure mode a generated directory is
    supposed to make impossible.
    """
    if not IMAGES_SRC.is_dir():
        return 0
    IMAGES_DST.mkdir(parents=True, exist_ok=True)
    for src in sorted(IMAGES_SRC.glob("*.png")):
        dst = IMAGES_DST / src.name
        if not dst.exists() or dst.read_bytes() != src.read_bytes():
            shutil.copy2(src, dst)
    for stale in IMAGES_DST.glob("*.png"):
        if not (IMAGES_SRC / stale.name).exists():
            stale.unlink()
    return len(list(IMAGES_DST.glob("*.png")))
...
def main(argv=None) -> int:
    ...
    n = sync_images()
    if n:
        print(f"copied {n} concept diagram(s) into docs/images/")
